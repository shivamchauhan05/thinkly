...
export default function CourseRegistrationForm() {
  ...
  const handleSubmit = async (e) => {
    ...
    setSubmitting(true);
    try {
      // FormData use kar rahe hain kyunki file bhi bhejni hai
      ...
      const res = await fetch('/api/send-email', {
        method: 'POST',
        body: payload, // Content-Type header mat set karo, browser khud set karega (multipart boundary ke sath)
      });
      ...
      <div id="apply" className="min-h-screen bg-gradient-to-br from-slate-50 to-blue-50 py-16 px-4 font-sans antialiased">
        <div className="max-w-4xl mx-auto">
          <div className="bg-white rounded-2xl shadow-xl p-12 text-center">
            ...
            <button
              onClick={() => { setSubmitted(false); setFormData({ name: '', email: '', phone: '', college: '', currentYear: '', studypassed: '', internshipDomain: '', tenure: '', whyJoin: '', priorExperience: '', flexibleSchedule: '', file: null }) }}
        ...
        <form onSubmit={handleSubmit} className="bg-white rounded-2xl shadow-xl shadow-gray-200/50 p-8 md:p-10 space-y-6">
          ...
          <div className="grid md:grid-cols-2 gap-5">
            <div>
             ...
             <div>
              <label className={labelClass}>Current year of study passed <span className="text-red-500">*</span></label>
              <input type="text" name="studypassed" required placeholder="" value={formData.studypassed} onChange={handleChange} className={inputClass} />
            </div>
            <div>
          ...
          <div>
            ...
            <p className="text-xs text-gray-400 mb-3">Upload your resume (PDF, DOC, DOCX — max 5MB).</p>
